...
function MedicoTableCard({
  medico,
  dados,
  onDeleteClick,
  globalSearch,
}: {
  medico: { label: string; value: string };
  dados: Lancamento[];
...
export default function LivroCaixaPage() {
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true);
    try {
      // Passamos undefined (ou null) para skip e limit no service, ou configuramos o service para ignorá-los
      const data = await financeiroService.getLancamentos(
        range.start.toString(),
        range.end.toString(),
        0, // skip
        10000 // Limite alto de segurança (o mês inteiro)
      );
      setLancamentos(data);
    } catch (error) {
      console.error(error);
      ...
      <AlertDialog open={!!lancamentoToDelete} onOpenChange={(open) => !open && setLancamentoToDelete(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={(e) => {
                e.preventDefault(); // Evita que o modal feche imediatamente
                handleDeleteConfirm();
              }}
